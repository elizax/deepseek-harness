...
`baseURL` sets the endpoint of every model on the route, so private proxies such as `https://proxy.example.com:8443` remain supported; a catalog route that omits it keeps each catalog model's own endpoint. Naming `api` on a catalog route repoints the whole route at that protocol, which is how a deployment moves a provider between, say, Responses and Chat Completions. `proxy` routes every request of the route through an HTTP(S) relay — the request URL stays `baseURL`, only the transport goes via the proxy — which is how a route reaches a gateway bound to a remote loopback address (an `HTTP_PROXY` for that route alone). It is honored by the pi-ai protocols (`anthropic-messages`, `openai-completions`, `openai-responses`) and by model discovery for a proxy-configured route.
